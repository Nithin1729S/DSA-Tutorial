#include <bits/stdc++.h>
using namespace std;

vector<int> rabin_karp(string const& s, string const& t) {
    const int p = 31;   // base of rabin karp
    const int m = 1e9 + 9;  //mod thingy to avoid overflow

    int S = s.size();
    int T = t.size();

    vector<long long> p_pow(max(S, T));

    p_pow[0] = 1;
    for (int i = 1; i < (int)p_pow.size(); i++)
        p_pow[i] = (p_pow[i-1] * p) % m;

    vector<long long> h(T + 1, 0);
    for (int i = 0; i < T; i++)
        h[i+1] = (h[i] + (t[i] - 'a' + 1) * p_pow[i]) % m;

    long long h_s = 0;
    for (int i = 0; i < S; i++)
        h_s = (h_s + (s[i] - 'a' + 1) * p_pow[i]) % m;

    vector<int> occurrences;
    
    for (int i = 0; i + S - 1 < T; i++) {
        long long cur_h = (h[i+S] + m - h[i]) % m;
        if (cur_h == h_s * p_pow[i] % m)
            occurrences.push_back(i);
    }
    return occurrences;
}

int main() {
    string s, t;

    cout << "Enter the pattern to search for: ";
    getline(cin, s); // Read the pattern string

    cout << "Enter the text to search in: ";
    getline(cin, t); // Read the text string

    vector<int> occurrences = rabin_karp(s, t);

    if (occurrences.empty()) {
        cout << "Pattern not found in the text.\n";
    } else {
        cout << "Pattern found at indices: ";
        for (int idx : occurrences) {
            cout << idx << " ";
        }
        cout << "\n";
    }

    return 0;
}
